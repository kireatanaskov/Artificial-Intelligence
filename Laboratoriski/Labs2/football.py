...
class Football(Problem):
    def __init__(self, opponents, initial, goal=None):
        ...
        self.controlled_by_opponent = ((2, 2), (3, 2), (4, 2),
                                       (2, 3), (4, 3), (2, 4),
                                       (3, 4), (4, 4), (5, 3),
                                       (6, 3), (6, 4), (4, 5),
                                       (5, 5), (6, 5))
    ...
    @staticmethod
    def check_valid(self, state, opponents):
        man_pos = state[0]
        ball_pos = state[1]

        man_x, man_y = man_pos[0], man_pos[1]
        ball_x, ball_y = ball_pos[0], ball_pos[1]

        # ako coveceto ispagja nadvor od granicite
        if man_x < 0 or man_x > 7:
            return False
        if man_y < 0 or man_y > 5:
            return False

        # ako topkata ispagja nadvor od granicite
        if ball_x < 0 or ball_x > 7:
            return False
        if ball_y < 0 or ball_y > 5:
            return False

        # ako topkata e na ista pozicija so protivnik
        if ball_pos in opponents:
            return False

        # ako topkata e na pole koe e sosedno so protivnicite
        if ball_pos in self.controlled_by_opponent:
            return False

        # ako coveceto se naogja na isto pole so topkata
        if man_pos == ball_pos:
            return False

        # ako coveceto se naogja na isto pole so nekoj od protivnicite
    ...
    @staticmethod
    def calculate_distance(ball_pos, goal):
        ball_x, ball_y = ball_pos
        goal_x, goal_y = goal

        return max(abs(goal_x - ball_x), abs(goal_y - ball_y))

    def h(self, node):
        ball_pos = node.state[1]
        goal = ((7, 2), (7, 2))

        return min(self.calculate_distance(ball_pos, goal[0]), self.calculate_distance(ball_pos, goal[1]))
...
if __name__ == '__main__':
    ...
    football = Football(opponents, (tuple(man_pos), tuple(ball_pos)))
