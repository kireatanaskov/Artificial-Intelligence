...
def new_state(head, body, direction, green_apples):
    apples1 = list(green_apples)
    body1 = list(body)

    if head in green_apples:
        """
        ako izela jabolko ne se brisi posledniot element od nizata
        tuku napred se dodava pozicijata na glavata 
        t.e. se izdolzuva teloto na zmijata
        """
        apples1.remove(head)
        body1.insert(0, head)
    else:
        """
        body = ((0, 7), (0, 8), (0, 9))
        body1 = body1[:1] go brisi posledniot element 
        vo slucajov (0, 9)
        ako zmijata prodolzi pravo od pocetnata pozicija
        glavata kje bide na pozicija (0, 6)
        i toa kje se dodade na pocetokot i nizata kje bide
        body = ((0, 6), (0, 7), (0, 8))
        """
        body1 = body1[:-1]
        body1.insert(0, head)

    return direction, tuple(body1), tuple(apples1)
...
class Snake(Problem):
    ...
    def successor(self, state):
        # state = (direction, (head_x, head_y), body, green_apples)
        ...
        if direc == "down":
            ...
            if head_y - 1 >= 0 and new_head_pos not in body:
                successors["ProdolzhiPravo"] = new_state(new_head_pos, body, direc, appless)

            new_head_pos = (head_x - 1, head_y)
            if head_x - 1 >= 0 and new_head_pos not in body:
                successors["SvrtiDesno"] = new_state(new_head_pos, body, "left", appless)

            new_head_pos = (head_x + 1, head_y)
            if head_x + 1 < 9 and new_head_pos not in body:
                successors["SvrtiLevo"] = new_state(new_head_pos, body, "right", appless)

        if direc == "up":
            new_head_pos = (head_x, head_y + 1)
            if head_y + 1 < 9 and new_head_pos not in body:
                successors["ProdolzhiPravo"] = new_state(new_head_pos, body, direc, appless)

            new_head_pos = head_x - 1, head_y
            if head_x - 1 >= 0 and new_head_pos not in body:
                successors["SvrtiLevo"] = new_state(new_head_pos, body, "left", appless)

            new_head_pos = head_x + 1, head_y
            if head_x + 1 < 9 and new_head_pos not in body:
                successors["SvrtiDesno"] = new_state(new_head_pos, body, "right", appless)
        ...
        if direc == "left":
            ...
            new_head_pos = (head_x, head_y - 1)
            if head_y - 1 >= 0 and new_head_pos not in body:
                successors["SvrtiLevo"] = new_state(new_head_pos, body, "down", appless)

            new_head_pos = (head_x, head_y + 1)
            if head_y + 1 < 9 and new_head_pos not in body:
                successors["SvrtiDesno"] = new_state(new_head_pos, body, "up", appless)
    ...
    def h(self, node):
        # state = (direction, head, body, apples)
        head = node.state[1]
        apples1 = node.state[3]
        sum_val = 0

        for apple1 in apples1:
            val = abs(head[0] - apple1[0]) + abs(head[1] - apple1[1])
            sum_val += val
...
if __name__ == "__main__":
    ...
    snake = Snake(("down", (0, 7), tuple(snakePos), tuple(apples)))
