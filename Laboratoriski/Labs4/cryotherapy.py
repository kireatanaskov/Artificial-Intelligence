...
if __name__ == '__main__':
    ...
    # se kastiraat podatocite vo float
    dataset = [[float(dataset[i][j]) for j in range(0, len(dataset[i]))] for i in range(0, len(dataset))]

    # se deli mnozestvoto na podatoci
    training_set = dataset[0:math.floor(0.85 * len(dataset))]
    testing_set = dataset[math.floor(0.85 * len(dataset)):]

    X = [training_set[i][:-1] for i in range(0, len(training_set))]  # vo X se samo karakteristikite od training set
    Y = [training_set[i][-1] for i in range(0, len(training_set))]  # vo Y e samo klasata od training setot

    # X = [[float(X[i][j]) for j in range(0, len(X[i]))] for i in range(0, len(X))]
    # Y = [[int(Y[i][j]) for j in range(0, len(Y[i]))] for i in range(0, len(Y))]

    clf = GaussianNB()
    clf.fit(X, Y)

    # se zimaat samo karakteristikite od testing set
    testing_set_x = [testing_set[i][:-1] for i in range(0, len(testing_set))]
    ...
    entry_prediction = clf.predict([entry])[0]
