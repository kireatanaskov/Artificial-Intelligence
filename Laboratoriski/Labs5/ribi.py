...
if __name__ == '__main__':
    ...
    train_set = dataset[0:math.floor(0.85 * len(dataset))]
    train_set_x = [row[:-1] for row in train_set]
    train_set_y = [row[-1] for row in train_set]

    test_set = dataset[math.floor(0.85 * len(dataset)):]
    test_set_x = [row[:-1] for row in test_set]
    test_set_y = [row[-1] for row in test_set]

    # se brise kolonata so indeks procitan od input
    train_x_rem = list()
    for t in train_set_x:
        sample = [t[i] for i in range(len(t)) if i != col_index]
        train_x_rem.append(sample)

    # se brise kolonata so indeks procitan od input
    test_x_rem = list()
    for t in test_set_x:
        sample = [t[i] for i in range(len(t)) if i != col_index]
        test_x_rem.append(sample)

    classifier = RandomForestClassifier(n_estimators=no_trees,criterion=criterion_input,random_state=0)
    classifier.fit(train_x_rem, train_set_y)  # se trenira klasifikatorot

    correct_samples = 0
    for x, y in zip(test_x_rem, test_set_y):
        y_predicted = classifier.predict([x])[0]
        if y_predicted == y:
            correct_samples += 1

    print(f'Accuracy: {correct_samples / len(test_set_y)}')

    test_sample = input()
    test_sample_float = [float(num_str) for num_str in test_sample.split()]

    test_sample_rem = [test_sample_float[i] for i in range(len(test_sample_float)) if i != col_index]

    # print(test_sample_rem)

    print(classifier.predict([test_sample_rem])[0])

    print(classifier.predict_proba([test_sample_rem])[0])  # verojatnosti za pripadnost vo klasite na noviot zapis
