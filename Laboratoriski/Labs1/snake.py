...
def move_down(head_x, head_y, red_apple_positions):
    if head_y > 0 and (head_x, head_y - 1) not in red_apple_positions:
        head_y -= 1
    return head_y


def move_up(head_x, head_y, red_apple_positions):
    if head_y < 9 and (head_x, head_y + 1) not in red_apple_positions:
        head_y += 1
    return head_y


def move_right(head_x, head_y, red_apple_positions):
    if head_x < 9 and (head_x + 1, head_y) not in red_apple_positions:
        head_x += 1
    return head_x


def move_left(head_x, head_y, red_apple_positions):
    if head_x > 0 and (head_x - 1, head_y) not in red_apple_positions:
        head_x -= 1
    return head_x


def is_it_colliding(head_x, head_y, body):
    if (head_x, head_y) not in body:
        return False
    return True


def eat(head_x, head_y, apples):
...
class Snake(Problem):
    ...
    def successor(self, state):
        # state = ([head_x, head_y], body, direction, green_apples)
        successors = dict()
    ...
    def goal_test(self, state):
        ...
        return len(green_apples) == 0  # ako nema povekje jabolki vo listata, vrati true
...
if __name__ == '__main__':
    ...
    for i in range(num_green):
        ...
        greenApples.append(p)

    for i in range(num_green):
        greenApples[i] = tuple(greenApples[i])
    ...
    for i in range(num_red):
        redApples[i] = tuple(redApples[i])

    state = ("down", ((0, 7), (0, 8), (0, 9)), tuple(greenApples))
