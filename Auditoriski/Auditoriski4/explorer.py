...
from searching_framework.informed_search import astar_search, best_first_graph_search, recursive_best_first_search, \
...
def update_obstacle_position(position):
    ...
    if (y == 0 and direction == -1) or (y == 5 and direction == 1):
        direction = direction * (-1)  # da se smeni nasokata ako e na kraj na tablata
    y_new = y + direction
...
class Explorer(Problem):
    ...
    def h(self, node):
        x = node.state[0]  # x na coveceto
        y = node.state[1]  # y na coveceto
        x1 = self.goal[0]  # x na kukjickata
        y1 = self.goal[1]  # y na kukjickata
...
if __name__ == "__main__":
    ...
    answer = greedy_best_first_graph_search(explorer)
