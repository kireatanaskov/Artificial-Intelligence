...
if __name__ == '__main__':
    ...
    training_set = newDataset[:int(0.85 * len(newDataset))]
    testing_set = newDataset[int(0.85 * len(newDataset)):]
    ...
    classifier = RandomForestClassifier(num_trees, criterion=criterion, random_state=0)
    classifier.fit(training_set_x, training_set_y)

    correct = 0
    predictions = classifier.predict(testing_set_x)
    for predicted, real in zip(predictions, testing_set_y):
        if predicted == real:
            correct += 1
    ...
    newEntry = [entry[i] for i in range(len(entry)) if i != col_index]

    print(f"Adccuracy: {accuracy}")
    print(classifier.predict([newEntry])[0])
    print(classifier.predict_proba([newEntry])[0])
