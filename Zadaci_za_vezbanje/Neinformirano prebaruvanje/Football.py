...
def moveFootballer(man_pos, ball_pos, direction):
    afterMove = []
    if (man_pos[0], man_pos[1] + 1) != (ball_pos[0], ball_pos[1]) and (man_pos[0], man_pos[1] + 1) not in opponents:
        afterMove = [man_pos[0], man_pos[1] + 1]
    elif man_pos[1] > ball_pos[1] and (man_pos[0], man_pos[1] - 1) != tuple(ball_pos):
        afterMove = [man_pos[0], man_pos[1] - 1]
    elif man_pos[0] < ball_pos[0] and (man_pos[0] + 1, man_pos[1]) != tuple(ball_pos):
        afterMove = [man_pos[0] + 1, man_pos[1]]
    elif man_pos[0] > ball_pos[0] and (man_pos[0] - 1, man_pos[1]) != tuple(ball_pos):
        afterMove = [man_pos[0] - 1, man_pos[1]]
...
class Football(Problem):
    ...
    def check_valid(self, state):
        man_pos = state[0]
        ball_pos = state[1]
        if (man_pos[0], man_pos[1]) == (ball_pos[0], ball_pos[1]): return False
        if man_pos[0] < 0 or man_pos[1] > 5: return False
        if ball_pos[0] < 0 or ball_pos[1] > 5: return False
        if (man_pos[0], man_pos[1]) in opponents: return False
    ...
    def successor(self, state):
        ...
        if man_pos[0] != x - 1 and man_pos[1] != y:
            if man_pos[0] < x - 1:
                new_man_pos = moveFootballer(man_pos, ball_pos, "desno")
                if self.check_valid((man_pos, ball_pos)):
                    successors[f"Pomesti coveche desno"] = (tuple(new_man_pos), tuple(ball_pos))

            if man_pos[0] >= x:
                new_man_pos = moveFootballer(man_pos, ball_pos, "levo")
                if self.check_valid((man_pos, ball_pos)):
                    successors[f"Pomesti coveche levo"] = (tuple(new_man_pos), tuple(ball_pos))

            if man_pos[1] < y:
                new_man_pos = moveFootballer(man_pos, ball_pos, "gore")
                if self.check_valid((man_pos, ball_pos)):
                    successors[f"Pomesti coveche gore"] = (tuple(new_man_pos), tuple(ball_pos))

            if man_pos[1] > y:
                new_man_pos = moveFootballer(man_pos, ball_pos, "dolu")
                if self.check_valid((man_pos, ball_pos)):
                    successors[f"Pomesti coveche dolu"] = (tuple(new_man_pos), tuple(ball_pos))

        if (x + 1, y) not in self.opponents:
            new_ball_pos = moveBall((x, y), "desno")
            if self.check_valid((man_pos, new_ball_pos)):
                successors["Turni topka desno"] = (tuple(man_pos), tuple(new_ball_pos))

        if (x, y + 1) not in self.opponents:
            new_ball_pos = moveBall((x, y), "gore")
            if self.check_valid((man_pos, new_ball_pos)):
                successors["Turni topka gore"] = (tuple(man_pos), tuple(new_ball_pos))

        if (x, y - 1) not in self.opponents:
            new_ball_pos = moveBall((x, y), "dolu")
            if self.check_valid((man_pos, new_ball_pos)):
                successors["Turni topka dolu"] = (tuple(man_pos), tuple(new_ball_pos))

        if (x + 1, y + 1) not in self.opponents:
            new_ball_pos = moveBall((x, y), "goredesno")
            if self.check_valid((man_pos, new_ball_pos)):
                successors["Turni topka gore-desno"] = (tuple(man_pos), tuple(new_ball_pos))

        if (x + 1, y - 1) not in self.opponents:
            new_ball_pos = moveBall((x, y), "doludesno")
            if self.check_valid((man_pos, new_ball_pos)):
                successors["Turni topka dolu-desno"] = (tuple(man_pos), tuple(new_ball_pos))
...
if __name__ == '__main__':
    ...
    game = Football(opponents, (man_pos, ball_pos))
    solution = breadth_first_graph_search(game).solution()
    print(solution)
