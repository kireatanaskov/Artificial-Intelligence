...
class Disks(Problem):
    ...
    def successor(self, state):
        ...
        for i in range(self.size):
            # Move one right
            if disks1[i] == 1000: continue
            if i + 1 < self.size and disks1[i + 1] == 1000:
                newArr = list(disks1)
                newArr[i + 1] = newArr[i]
                newArr[i] = 1000

                successors[f"D1: Disk {disks1[i]}"] = tuple(newArr)
            # Move two right
            if i + 2 < self.size and disks1[i + 2] == 1000 and disks1[i + 1] != 1000:
                newArr = list(disks1)
                newArr[i + 2] = newArr[i]
                newArr[i] = 1000

                successors[f"D2: Disk {disks1[i]}"] = tuple(newArr)

            # Move one left
            if i > 0 and disks1[i - 1] == 1000:
                newArr = list(disks1)
                newArr[i - 1] = newArr[i]
                newArr[i] = 1000

                successors[f"L1: Disk {disks1[i]}"] = tuple(newArr)

            # Move two left
            if i > 1 and disks1[i - 2] == 1000 and disks1[i - 1] != 1000:
                newArr = list(disks1)
                newArr[i - 2] = newArr[i]
                newArr[i] = 1000

                successors[f"L2: Disk {disks1[i]}"] = tuple(newArr)
...
if __name__ == "__main__":
    ...
    for i in range(1, L + 1):
        disks.append(i) if i <= N else disks.append(1000)

    goalState = sorted(disks, reverse=True)

    problem = Disks(L, tuple(disks), tuple(goalState))
