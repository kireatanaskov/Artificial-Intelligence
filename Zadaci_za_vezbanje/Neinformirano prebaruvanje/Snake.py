...
def check_eaten_apple(snakeHead, greenApples):
    return snakeHead in greenApples


def move_forward(body, head, direction, greenApples):
    afterMove = []
    if direction == "up":
    ...
    if check_eaten_apple(tuple(afterMove), tuple(greenApples)):
        greenApples.remove(tuple(afterMove))
    else:
        body.pop(0)

    body.insert(-1, (head[0], head[1]))
    head = (afterMove[0], afterMove[1])
    return body, head, direction, greenApples


def turn_left(body, head, direction, greenApples):
    if direction == "up":
        return move_forward(body, head, "left", greenApples)
    elif direction == "down":
        return move_forward(body, head, "right", greenApples)
    elif direction == "left":
        return move_forward(body, head, "down", greenApples)
    elif direction == "right":
        return move_forward(body, head, "up", greenApples)

    return body, head, direction, greenApples


def turn_right(body, head, direction, greenApples):
    if direction == "up":
        return move_forward(body, head, "right", greenApples)
    elif direction == "down":
        return move_forward(body, head, "left", greenApples)
...
class Snake(Problem):
    ...
    def successor(self, state):
        ...
        new_state = move_forward(list(snake_body), list(snake_head), direction, list(green_apples))
        if self.check_valid(new_state):
            successors["ProdolzhiPravo"] = (tuple(new_state[0]), tuple(new_state[1]), new_state[2], tuple(new_state[3]))

        new_state = turn_right(list(snake_body), list(snake_head), direction, list(green_apples))
        if self.check_valid(new_state):
            successors["SvrtiDesno"] = (tuple(new_state[0]), tuple(new_state[1]), new_state[2], tuple(new_state[3]))

        new_state = turn_left(list(snake_body), list(snake_head), direction, list(green_apples))
        if self.check_valid(new_state):
            successors["SvrtiLevo"] = (tuple(new_state[0]), tuple(new_state[1]), new_state[2], tuple(new_state[3]))
    ...
    def check_valid(self, state):
        ...
        if (head[0], head[1]) in body or (head[0], head[1]) in self.red_apples: return False
...
if __name__ == '__main__':
    ...
    snake = Snake(red_apples, (snake_body, snake_head, direction, tuple(green_apples)))

    sol = breadth_first_graph_search(snake).solution()
