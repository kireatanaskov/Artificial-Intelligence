...
class Disks(Problem):
    ...
    def successor(self, state):
        ...
        for i in range(self.size):
            ...
            if i + 1 < self.size and disks[i + 1] == 1000:
                ...
                successors[f"D1: Disk {disks[i]}"] = tuple(newArr)

            # Move two right
            if i + 2 < self.size and disks[i + 1] != 1000 and disks[i + 2] == 1000:
                newArr = list(disks)
                newArr[i + 2] = newArr[i]
                newArr[i] = 1000

                successors[f"D2: Disk {disks[i]}"] = tuple(newArr)
            ...
            if i > 0 and disks[i - 1] == 1000:
                ...
                successors[f"L1: Disk {disks[i]}"] = tuple(newArr)

            # Move two left
            if i > 1 and disks[i - 1] != 1000 and disks[i - 2] == 1000:
                newArr = list(disks)
                newArr[i - 2] = newArr[i]
                newArr[i] = 1000

                successors[f"L2 Disk {disks[i]}"] = tuple(newArr)
    ...
    def h(self, node):
        ...
        for x, y in zip(disks, self.goal):
            if x != y:
                count += 1
...
if __name__ == "__main__":
    ...
    solution = recursive_best_first_search(problem).solution()
