...
def check_pellets(position, pellets):
...
def moveForward(pacman, pellets):
    ...
    if check_pellets(tuple(afterMove), tuple(pellets)):
...
def moveBackward(pacman, pellets):
    direction = pacman[2]
    if direction == "istok":
        return moveForward((pacman[0], pacman[1], "zapad"), pellets)
    elif direction == "zapad":
...
class Pacman(Problem):
    ...
    def successor(self, state):
        ...
        if self.check_valid(newState):
            successors["ProdolzhiPravo"] = (tuple(newState[0]), tuple(newState[1]))

        newState = moveBackward(pacman, list(pellets))
        if self.check_valid(newState):
            successors["ProdolzhiNazad"] = (tuple(newState[0]), tuple(newState[1]))

        newState = turnLeft(pacman, list(pellets))
        if self.check_valid(newState):
            successors["SvrtiLevo"] = (tuple(newState[0]), tuple(newState[1]))

        newState = turnRight(pacman, list(pellets))
        if self.check_valid(newState):
            successors["SvrtiDesno"] = (tuple(newState[0]), tuple(newState[1]))
...
if __name__ == "__main__":
    obstaclesList = ((6, 0), (4, 1), (5, 1), (6, 1), (8, 1),
                     (1, 2), (6, 2), (1, 3), (1, 4), (8, 4),
                     (9, 4), (4, 5), (0, 6), (3, 6), (4, 6),
                     (5, 6), (4, 7), (8, 7), (9, 7), (0, 8),
                     (8, 8), (9, 8), (0, 9), (1, 9), (2, 9),
    ...
    pacman_position = (x, y, direction)

    numPellets = int(input())
    pellets = []

    for i in range(0, numPellets):
        pellets.append(tuple(map(int, input().split(","))))

    problem = Pacman(obstaclesList, (pacman_position, tuple(pellets)))
