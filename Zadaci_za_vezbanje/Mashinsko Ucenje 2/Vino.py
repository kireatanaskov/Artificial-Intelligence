...
if __name__ == '__main__':
    ...
    for row in dataset:
        if row[-1] == 0:
            zeroClass.append(row)
        elif row[-1] == 1:
            oneClass.append(row)
        else:
            twoClass.append(row)

    training_set_NB = zeroClass[:int(x1 * len(zeroClass))] + oneClass[:int(x1 * len(oneClass))] + twoClass[:int(x1 * len(dataset))]
    training_set_DT = zeroClass[int(x1 * len(zeroClass)):int(x2 * len(zeroClass))] + oneClass[int(x1 * len(oneClass)):int(x2 * len(oneClass))] + twoClass[int(x1 * len(twoClass)):int(x2 * len(twoClass))]
    training_set_RF = zeroClass[:int(x2 * len(zeroClass))] + oneClass[:int(x2 * len(oneClass))] + twoClass[:int(x2 * len(twoClass))]
    testing_set = zeroClass[int(x2 * len(zeroClass)):] + oneClass[int(x2 * len(oneClass)):] + twoClass[int(x2 * len(twoClass)):]

    NB_x, NB_y = split_set(training_set_NB)
    DT_x, DT_y = split_set(training_set_DT)
    RF_x, RF_y = split_set(training_set_RF)
    TS_x, TS_y = split_set(testing_set)

    NB_classifier = GaussianNB()
    DT_classifier = DecisionTreeClassifier(random_state=0)
    RF_classifier = RandomForestClassifier(n_estimators=3, random_state=0)

    NB_classifier.fit(NB_x, NB_y)
    DT_classifier.fit(DT_x, DT_y)
    RF_classifier.fit(RF_x, RF_y)

    predictions_NB = NB_classifier.predict(TS_x)
    predictions_DT = DT_classifier.predict(TS_x)
    predictions_RF = RF_classifier.predict(TS_x)

    correct = 0
    for nb, dt, rf, real in zip(predictions_NB, predictions_DT, predictions_RF, TS_y):
        count = 0
        if nb == real:
            count += 1
        if dt == real:
